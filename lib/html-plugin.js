...
class HTMLPlugin {
  ...
  plugin (compilation, callback) {
    ...
    const prefetch = generatePrefetchLinks(chunks.js, publicPath)
...
const generatePrefetchLinks = (files = [], publicPath = '') => files
  .map(file => `<link rel='prefetch' as='script' href='${publicPath + file}'>`)
  .join('')
