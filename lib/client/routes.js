...
const context = require.context(__DIRNAME__, true, /\.(js|md|mdx)$/, 'sync')
const getAsync = require.context(__DIRNAME__, true, /\.(js|md|mdx)$/, 'lazy')
...
export const routes = context.keys().map(key => {
  ...
  return {
    ...
    path: pathname,
    // ...exports,
    Component: lazyComponent(() => __STATIC__ ? context(key) : getAsync(key)),
  }
}).filter(Boolean)
